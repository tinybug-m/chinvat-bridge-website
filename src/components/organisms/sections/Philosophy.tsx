...
const PILLARS = ["Strategy", "Engineering", "Automation", "Measurable Outcomes"];

export function Philosophy() {
  return (
    <Section id="about" relative containerSize="5xl">
      <CornerFrame className="border border-stone-border bg-obsidian-900/60 p-8 sm:p-14 rounded-sm">
        <div className="text-center space-y-6 max-w-3xl mx-auto">
          ...
          <BulletRow
            className="pt-4 justify-center font-mono text-[10px] tracking-technical text-parchment-dim uppercase"
            items={PILLARS}
            renderItem={(pillar, index) =>
              index === PILLARS.length - 1 ? (
                <span className="text-gold-400 font-semibold">{pillar}</span>
              ) : (
                pillar
              )
            }
